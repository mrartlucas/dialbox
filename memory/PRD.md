...
## Update 2026-07-21 (Cell2Jack / physical-keypad readiness — Iterations 17-18 VERIFIED 100%)
- **Phase 1 — Shared keypad input architecture:** added `frontend/src/lib/dialboxKeys.js` (VALID_KEYS 0-9*#, VALID_SOURCES screen|computer-keyboard|voice|dtmf|telephone|test, keyboard→key mapping, typing-context guard, dev-keyboard localStorage helpers, CustomEvent bridge). `PhoneSimulator.jsx` now has `dispatchDialBoxKey(key, source)` — the SINGLE entry into `onKey()`. On-screen `Keypad` (source 'screen'), voice number commands (source 'voice'), a new computer-keyboard listener (source 'computer-keyboard', off-hook + not-typing gated, default ON in dev), and a window `dialbox-external-key` bridge (dtmf/test) all route through it — no duplicated menu logic.
- **Phase 2 — Cell2Jack / DTMF Test Lab:** `frontend/src/lib/dtmf.js` = pure, UI-free Goertzel detector (`detectDtmfFromSamples`, stateful `DtmfDetector` with min-duration/debounce/duplicate-rejection/noise-floor/twist/confidence, `synthesizeDtmf`). `components/DtmfTestLab.jsx` (opened from a header button → shadcn Dialog): mic permission, device enumeration/selection, start/stop, level meter, last-detected key, 12-key test grid, sensitivity slider, 'Accept DTMF input' toggle, 'Enable Development Keyboard Controls' toggle, per-key + 'Run simulated test' generator (synthesizes real DTMF via Web Audio oscillators → same analyser/Goertzel loop, no hardware), full audio cleanup, and the required Cell2Jack limitations notice. Detected keys (when accept on) feed the live box via `emitExternalKey(key,'dtmf')`.
- **Phase 3 — Docs:** `docs/README.md` index + `docs/hardware/` {HARDWARE_SHOPPING_LIST (moved), CELL2JACK_LIMITATIONS, TELEPHONE_INTEGRATION_PLAN, DTMF_DETECTION, HARDWARE_ROADMAP}.
- **Tests:** `frontend/src/lib/dtmf.test.js` — 24 Jest unit tests (all 12 keys, min-duration, debounce, duplicate rejection, noise tolerance, invalid-pair rejection) ALL PASS. Integration via testing_agent iteration_17/18: keypad regression, computer-keyboard nav (default ON), DTMF Lab UI, simulated 12/12 detection, dtmf→dispatch bridge, mic permission, cleanup, a11y — 100%. Fixed: isDevBuild() browser-bundle bug (use `process.env.NODE_ENV`), added DialogDescription.
- **All 10 deliverables complete.** No telephony provider implemented (planning doc only, per instructions).
- **STILL OVERDUE:** split PhoneSimulator.jsx (~2290 lines) into per-oracle modules — flagged again by code review in iteration_17.
